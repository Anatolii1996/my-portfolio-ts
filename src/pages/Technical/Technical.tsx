  ...
  return (
    <div className={pageAnimStyle}>
      ...
      <div className="history_updates">
        ...
        <h3>1.0.1</h3>
       <ul>
        <li>індекси кожної сторінки винесені в окремі змінні, що сприятиме підвищенню простоти розуміння коду в майбутньому</li>
        <li>посилання на сервер винесено в змінну оточення, що підвищує безпеку додатку</li>
       ...
       <ul>
        ...
        <li>додав в галерею проєкт над яким працював останнім часом. Це сервіс для замовлення їжі. На момент додавання проєкт ще є досить сирим, проте вважаю, що на MVP тягне. Далі необхідно обробити час завантаження зображень з сервера, обробити всі можливі помилки при спілкуванні з сервером. Навести лад з JWT авторизацією. Так я додав сюди авторизацію токенами (на момент публікації логіка, ще не завершена). Проєкт зроблено на прикладі реальної компанії, на момент публікації проект ще не показано потенційному клієнту. Спочатку хочу навести лад. Якщо клієнт захоче отримати собі цей продукт - значить він стане моїм першим комерційним проєктом, якщо ні буду на ньому далі тренуватись (напишу тести, перепишу backend на Nest, можливо заміню React Saga на RTK Query та відрефакторю код)</li>
       </ul>
        </>:<>
       ...
       <ul>
        ...
        <li>removed the "Other Projects" section from the gallery because I realized that no potential employer would be impressed by a JavaScript calculator or a ToDo list. Understanding the trends in the labor market and expectations from a candidate, it's probably better not to show such projects to anyone at all.</li>
         <li>added to the gallery a project I've been working on recently. It's a food delivery service. At the time of adding it, the project was still quite raw, but I think it's good enough for MVP. Next, we need to work out the time it takes to load images from the server, and process all possible errors when communicating with the server. I also need to fix the JWT authorization. So I added token authorization here (at the time of publication, the logic is not yet complete). The project is based on the example of a real company, and at the time of publication, the project has not yet been shown to a potential client. I want to put things in order first. If the client wants to get this product, then it will become my first commercial project, otherwise I will continue to train on it (write tests, rewrite the backend to Nest, possibly replace React Saga with RTK Query and refactor the code)</li>
       </ul>
        </>}
