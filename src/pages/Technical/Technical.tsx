  ...
  return (
    <div className={pageAnimStyle}>
      <p>
        ...
        ресурс, а також для потенційного роботодавця, який хоче зрозуміти якими
      ...
      <p>
        ...
        вивчення документації.</p> 
        <p>
          Найсуттєвішою такою технологією став TypeScript.
          Власне проєкт був заново переписаний на TypeScript коли вже було
          створено близько 30% кодової бази. Рішення переводити проєкт на TS було
          продиктовано високою популярністю цієї мови, та вимогою знання TS в 90%
          вакансій frontend-розробника.
        </p>
      ...
      <p>
        ...
        увагу на Node.js. Створення backend логіки мене захопило не меньш ніж
      ...
      <ul>
        <li>
          ...
          <p>
            ...
            полетіти в космос, сфотографувати там теслу, що літає на орбіті, та
        ...
        <li>
          ...
          <p>Під час роботи над даним проєктом, відбулось моє перше знайомство з цією мовою. Тому хто з Вас буде дивитись мій код, не судіть строго).</p>
          <p>В цілому мої враження від використання TS позитивні.</p>
            <p>Як було сказано вище, починав писати проєкт я на JS. Потім вирішив переписати та продовжувати роботу на TS, причин тому декілька:</p>
            <ol>
              <li>По-перше у вакансіях всі хочуть знання TS, а ринок праці сильніше за все диктує розробникам які технології вчити</li>
              <li>По-друге ця мова зараз стрімко набирає популярність, а в Європі це чи не найпопулярніша мова програмування</li>
              <li>По-третє знання TS дозволяє опановувати інші фреймворки такі як Angular. І хоча я зараз не планую вчити Angular, все ж в майбутньому не відкидаю таку можливість. А от наприклад Nest збираюсь вчити відразу після того як покрию код тестами, а там також TS (ще й ООП), тому знання TypeScript вважаю зараз обов'язковим для Frontend/FullStack розробника.</li>
            </ol>
        </li>
        <li>
        ...
        <li>
          ...
          <p>Також, під час завантаження додатку, в стор записую всю необхіжну інформацію з бази даних, що підвищує швидкість роботи додатку.</p>
        </li>
        <li>
          <div className="technical__label">
          ...
          <p>
           Бібліотека з екосистеми редаксу. Найперший спосіб написання асинхронного коду декларативно, що я побачив. Засновано на функціях-генераторах, що є нетривіальним розділом JavaScript, тому потребує трохи часу на вивчення. Зате зрозумівши як це працює, я отримав зручний спосіб керування асинхронним кодом. Логіка асинхронних викликів стала лаконічною та зрозумілою.         
          </p>
          <p>
          Знаю, що зараз такий спосіб керування асинхронним кодом є застарілим, адже після появи reduxjs/toolkit всі перейшли на RTK Query. Я також дивився в цю сторону, з використанням RTK Query коду стає менше, але на мою думку код не такий зрозумілий і не так тонко можна керувати асинхронними викликами. Загалом Redux-Saga мені дуже подобається.
          </p>
          <p>
          А в який спосіб Ви керуєте асинхронним кодом? Невже Redux-Saga вже є безнадійно застарілим?
          </p>
        </li>
        <li>
          <div className="technical__label">
          ...
          <p>Найбільший UI-фреймворк, що я бачив. Всі UI-компоненти в даному проєкті, взяті звідти (крім форми). Живий продукт який постійно розвивається, та має дуже широкий функціонал. Код написаний відразу в двох варіантах, на JS та TS, обирай, що треба. Має дуже великі можливості для кастомізації компонентів, проте не безмежні. Через велику базу, потребує часу на вивчення, чим я зараз і займаюсь. Буду використовувати і надалі.</p>
        </li>
        ...
        <li>
          ...
          <p>
            ...
            раніше записані, а також обробка повідомлень в чаті. Дуже цікава та функціональна технологія. Розумію,
            що вивчати тут мені ще доведеться дуже багато, але обов'язково буду
            цим займатись. В якості Node.js фреймворку взято Express через його простоту та популярність. Код дуже схожий на звичайний Node.js, тому період звикання мінімальний. В майбутньому все ж планую перейти на Nest. Хто пише backend, що про це думаєте?
          </p>
        </li>
        {/* <li>
          <div className="technical__label">
          ...
          <p>Найпопулярніший Node.js фреймворк, що відрізняється своєю простотою та стабільністю. Код дуже схожий на звичайний Node.js, тому період звикання мінімальний. В майбутньому все ж планую перейти на Nest.</p>
        </li> */}
        <li>
          <div className="technical__label">
          ...
          <p>Перша база даних, з якою я так серйозно працював. Досить потужна NoSQL база даних. Має можливість працювати з даними локально або в хмарі. Має розвинену підтримку та багато зрозумілої документації. Має можливість задання строгої типізації для даних в колекціях, без TypeScript, що дозволяє зробити взаємодію з БД стабільною, та уникнути зайвих помилок. Як на мене то типізація працює не настільки круто як в TS, але така можливість все одно крута. Firebase мені не настільки сподобався. В майбутньому буду вважати основною БД, якщо не буде інших вимог. А якими базами даних користуєтесь Ви?</p>
        </li>
        {/* <li>
          <div className="technical__label">
            <h3>Mongoose</h3>
        ...
        <li>
          ...
          <p>Дуже функціональний безкоштовний хостинг. Має великий функціонал, розгортає проєкти прямо з репозиторію на льоту, може хостити backend додатки. Для початківця - те що треба. Для комерційної роботи не взяв би через ціну. Цікаво, який хостинг Ви вважаєте найкращим?</p>
        </li>
      </ul>
    </div>
  );
